...
| `done-gate.sh` remembers acknowledged findings | The gate repeated the same finding every stop; a session passed it three times with one phrase, on a line it never wrote, which is how a real leak in the same list gets waved through | 20 Sep 2026 | Once per finding: journal keyed by line hash, reason captured from the transcript, changed line blocks again; only lines this session wrote are judged |
| `secretscan.py` matches shape, not only brand | The brand list missed two providers in use; a detector that knows names cannot know the next name | 20 Sep 2026 | `key = value` and high-entropy-token rules alongside the brand list, shared by both gates |
| `publish-guard.sh` on PreToolUse | A parallel session ran `gh repo create --public --push`; the only gate that fired was the Stop hook, after the fact and about the wrong thing | 20 Sep 2026 | Tracked files scanned before the command runs; a clean scan is reported, a hit blocks |
